...
const InventoryPage: React.FC = () => {
  ...
  const fixImageUrl = (url: string | null): string | null => {
    ...
    // Trocar raspa.ae por api.raspenahora.site
    let fixedUrl = url.replace('https://raspa.ae/', 'https://api.raspenahora.site/');
  ...
  const fetchPendingRedemptions = async () => {
    ...
    try {
      setLoading(true);
      const response = await fetch('https://api.raspenahora.site/v1/api/users/redemptions/pending', {
        method: 'GET',
        headers: {
  ...
  const handleRedeem = async (gameId: string) => {
    ...
    try {
      const response = await fetch('https://api.raspenahora.site/v1/api/users/redemptions/choose', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
