    ...
    if (isOpen) {
      ...
      const checkStatus = async () => {
        try {
          ...
          const response = await fetch(`https://api.raspenahora.site/v1/api/deposits/${paymentId}/status`, {
            method: 'GET',
            headers: {
...
export default function DepositModal({ isOpen, onClose, token }: DepositModalProps) {
  ...
  const handleGeneratePayment = async () => {
    ...
    try {
      const response = await fetch('https://api.raspenahora.site/v1/api/deposits/create', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
